      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full">
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:border-brand-tealCyan text-sm bg-gray-50 focus:bg-white"
          />
        ...
        <select 
          ...
          className="px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:border-brand-tealCyan text-sm bg-gray-50 focus:bg-white w-full md:w-48"
        >
          <option value="">All Months</option>
          {months.map(m => <option key={m} value={m}>{m}</option>)}
        </select>

        <label className="flex items-center gap-2 cursor-pointer bg-brand-softLavender/10 px-4 py-2.5 rounded-xl border border-brand-softLavender/20 w-full md:w-auto shrink-0">
          <input 
            type="checkbox" 
            checked={showOnlyDues} 
            onChange={() => setShowOnlyDues(!showOnlyDues)}
            className="w-4 h-4 text-brand-royalPurple rounded border-gray-300 focus:ring-brand-royalPurple"
          />
          <span className="text-sm font-bold text-brand-deepPlum">Show Only Dues</span>
        </label>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto p-4 min-h-[400px]">
          <table className="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan="7" className="text-center py-10 text-gray-400 font-semibold">Loading reports...</td></tr>
              ) : filteredPayments.length === 0 ? (
                <tr><td colSpan="7" className="text-center py-10 text-gray-400 font-semibold">No records found matching your filters.</td></tr>
              ) : (
                filteredPayments.map(payment => (
                  <tr key={payment.id} className="hover:bg-gray-50 border-b border-gray-50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`px-2.5 py-1 rounded-md text-[11px] font-bold uppercase ${Number(payment.due_amount) > 0 ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'}`}>
                        ৳ {payment.due_amount}
                      </span>
                    </td>
                    ...
                    <td className="p-4 text-center">
                      <Link to={`/admin/receipt/${payment.id}`} className="text-brand-tealCyan hover:text-[#4bc2ab] font-bold text-lg transition-colors p-2 inline-block bg-white shadow-sm border border-gray-100 rounded-lg hover:shadow-md" title="View Receipt">
                        🖨️
                      </Link>
                    </td>
                  </tr>
